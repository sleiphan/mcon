...
int mcon_process_server_socket_event(struct mcon* mcon, const struct epoll_event epoll_event, struct mcon_event* events, unsigned int max_events) {
    const struct mcon_epoll_entry epoll_data = ((union mcon_epoll_data) epoll_event.data.u64).entry;
