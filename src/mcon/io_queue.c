...
int io_queue_create(struct io_queue *ioq, const uint32_t max_io_operations_in_flight) {
    ...
    *ioq = (struct io_queue){.queue = q};

    return 0;
}

void io_queue_destroy(struct io_queue *ioq) { kqueue_destroy(&ioq->queue); }
...
int io_queue_pop_into_ring(struct io_queue *ioq, struct io_uring *ring, const unsigned int count) {
    ...
    for (int priority_group = 0; priority_group < BLACKHOLE_IO_QUEUE_PRIORITY_GROUP_COUNT;
         priority_group++) {
